...
# Main crawl sequence
scoreboard players set @e[type=area_effect_cloud,tag=ezcrawl.area_effect_cloud] ezcrawl.active 0
execute as @a[scores={ezcrawl.crawling=0..}] at @s run function ezcrawl:zz_private/crawl/main



# If the player was unable to execute the crawl sequence (due to dimension change or quit), kill the area_effect_cloud
execute as @e[type=area_effect_cloud,tag=ezcrawl.area_effect_cloud,scores={ezcrawl.active=0}] run function ezcrawl:zz_private/crawl/kill

# All area_effect_clouds who lost their shulker kill themselves
execute as @e[type=area_effect_cloud,tag=ezcrawl.area_effect_cloud,predicate=!ezcrawl:has_passenger] run function ezcrawl:zz_private/crawl/kill
