...
execute unless score @s ezcrawl.settings.stop_angle matches -90..90 run scoreboard players set @s ezcrawl.settings.stop_angle -50



# If player moves, stop showing settings
execute if predicate ezcrawl:is_moving run scoreboard players set @s customizeCrawlAngles 0

# Let players run settings unless moving
scoreboard players enable @s customizeCrawlAngles
execute if predicate ezcrawl:is_moving run trigger customizeCrawlAngles add 0
